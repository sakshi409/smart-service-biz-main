...
const Login = () => {
  ...
  const [signupEmail, setSignupEmail] = useState("");
  const [signupPassword, setSignupPassword] = useState("");
  const [signupRole, setSignupRole] = useState("USER"); // NEW: Role state

  // Loading states
  const [isLoginLoading, setIsLoginLoading] = useState(false);
  const [isSignupLoading, setIsSignupLoading] = useState(false);

  // NEW: Dashboard redirect function based on role
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/register`, {
        ...
        body: JSON.stringify({
          ...
          email: signupEmail || null,
          password: signupPassword,
          role: signupRole, // NEW: Include role
        }),
      });
      ...
      if (response.ok && data.success) {
        ...
        // NEW: Redirect based on user role immediately after signup
        setTimeout(() => {
          redirectToDashboard(data.user.role);
        }, 1000);
      } else {
